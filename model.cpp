...
// https://github.com/kuyuri-iroha/Draw-PMX/blob/master/DrawPMX/pmx_loader.cpp
...
bool loadPMX(PMXModelData& data, const std::string& _filePath)
{
	...
	// ヘッダー -------------------------------
	std::array<uint8_t, 4> pmxHeader{};
	constexpr std::array<uint8_t, 4> PMX_MAGIC_NUMBER{ 0x50, 0x4d, 0x58, 0x20 };
	...
	uint8_t hederDataLength = pmxFile.get();
	if (hederDataLength != 8)
	{
		pmxFile.close();
		return false;
	}
	std::array<uint8_t, 8> hederData{};
	...
	for (int i = 0; i < numberOfVertex; i++)
	{
		...
		const uint8_t weightMethod = pmxFile.get();
		switch (weightMethod)
		{
	...
	for (int i = 0; i < numOfMaterial; i++)
	{
		...
		const uint8_t shareToonFlag = pmxFile.get();
		if (shareToonFlag)
		{
			pmxFile.get();
		}
